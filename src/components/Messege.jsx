...
const Messege = ({ msg }) => {
  ...
  const handleDelete = async () => {
    if (msg.file) {
      ...
      deleteObject(ref(storage, msg.file)).catch((error) => {
        toast.error("Failed to delete image from storage!!!", error);
      });
    }
      ...
      {msg.senderId === currentUser.uid ? (
        <div ref={ref} className="flex flex-row-reverse gap-2 mb-3">
          ...
          <div className="flex flex-col items-end max-w-[70%] gap-3 relative group">
            <button
              ...
              className="absolute pt-[2px] hidden group-hover:block text-gray-200 hover:text-red-500"
            >
              <MdDelete size={18} />
            </button>
            {msg?.file && (
              <img
                src={msg?.file}
                className="w-[85%] lg:w-[60%] -mb-2"
                alt="sent file"
                loading="lazy"
              />
            )}
            {msg?.text && (
              <p className="bg-chat text-white px-5 group-hover:pr-7 py-1 rounded-lg rounded-tr-none max-w-max">
                {msg.text}
              </p>
            )}
            {msg?.date && (
        ...
        <div ref={ref} className="flex gap-2 mb-3">
          ...
          <div className="flex flex-col max-w-[70%] gap-3 relative group">
            <button
              onClick={handleDelete}
              className="absolute pt-[2px] items-end hidden group-hover:block text-gray-400 hover:text-red-500"
            >
              <MdDelete size={18} />
            </button>
            {msg?.file && (
            ...
            {msg?.text && (
              <p className="bg-white px-5 group-hover:pl-7 py-1 rounded-lg rounded-tl-none max-w-max">
                {msg.text}
              </p>
            )}
            {msg?.date && (
